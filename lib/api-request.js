const debug = require('debug')('founded-api:request')
		...
		return new Promise((resolve, reject) => {
			...
			const retry = response => {
				if (response.statusCode === 204) {
					debug('polling: resource not ready')
					if (retryCount < this.pollMaxRetries) {
						retryCount++
						debug(`polling: try again in ${this.pollRate}ms`)
						setTimeout(() => {
							req()
						}, this.pollRate)
					} else {
						debug('polling: too many retries')
						reject(new Error('Too many polling retries'))
					}
				} else {
					debug('polling: resource received')
					resolve(response)
				}
			}
			debug('polling: begin')
